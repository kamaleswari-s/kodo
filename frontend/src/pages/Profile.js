...
export default function Profile() {
  ...
  const avatarColor = avatarColors[user?.name?.charCodeAt(0) % avatarColors.length] || '#E8572A';
  ...
  const roleStyle = roleColors[user?.role] || roleColors.member;
    ...
    <div style={{ maxWidth: '700px', margin: '0 auto' }}>
      ...
      <div style={{ background: theme.card, border: `0.5px solid ${theme.cardBorder}`, borderRadius: '16px', padding: '28px', marginBottom: '16px' }}>
        <div style={{ display: 'flex', alignItems: 'flex-start', gap: '20px', marginBottom: '24px' }}>
          <div style={{ width: '72px', height: '72px', borderRadius: '50%', background: avatarColor, color: '#fff', fontSize: '28px', fontWeight: 700, display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
            {user?.name?.charAt(0).toUpperCase()}
          </div>
          <div style={{ flex: 1 }}>
            <div style={{ fontFamily: 'Fraunces, serif', fontSize: '22px', fontWeight: 700, color: theme.text, marginBottom: '4px' }}>{user?.name}</div>
            <div style={{ fontSize: '13px', color: theme.textMuted, marginBottom: '6px' }}>{user?.email}</div>
        ...
        {editing ? (
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
            {user?.bio && (
              <div>
                <div style={{ fontSize: '11px', fontWeight: 600, letterSpacing: '0.06em', textTransform: 'uppercase', color: theme.textMuted, marginBottom: '6px' }}>Bio</div>
                <div style={{ fontSize: '14px', color: theme.textSecondary, lineHeight: 1.7 }}>{user.bio}</div>
              </div>
            )}
            ...
            {user?.github_url && (
              <div>
                ...
                <a href={user.github_url} target="_blank" rel="noopener noreferrer" style={{ fontSize: '13px', color: '#6C5CE7', textDecoration: 'none', fontFamily: 'JetBrains Mono, monospace' }}>
                  {user.github_url}
                </a>
              </div>
            )}
            ...
            {!user?.bio && !user?.github_url && (!user?.skills || user.skills.length === 0) && (
      ...
      <div style={{ background: theme.card, border: `0.5px solid ${theme.cardBorder}`, borderRadius: '16px', padding: '20px' }}>
        ...
        {[
          ...
          { label: 'Member since', value: user?.created_at ? new Date(user.created_at).toLocaleDateString('en-IN', { day: '2-digit', month: 'long', year: 'numeric' }) : 'N/A' },
        ].map((row) => (
