...
export default function Activity() {
  ...
  const fetchActivity = async () => {
    try {
      const res = await getActivity(currentWorkspace.id);
  ...
  return (
    <div>
      <div style={{ marginBottom: '24px', display: 'flex', alignItems: 'center', gap: '10px' }}>
        ...
        <div style={{ fontFamily: 'Fraunces, serif', fontSize: '28px', fontWeight: 700, color: theme.text }}>
          Activity feed
        </div>
      </div>
      ...
      {loading ? (
        ...
        <div style={{ textAlign: 'center', padding: '60px', background: theme.card, border: `0.5px solid ${theme.cardBorder}`, borderRadius: '14px' }}>
          ...
          <div style={{ fontSize: '13px', color: theme.textMuted }}>Actions across your workspace will appear here in real time</div>
        </div>
      ) : (
